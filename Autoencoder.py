...
def preprocess_states(states_list, n_states, batch_size):
    ...
    print(f"{len(states_list)} where preprocessed into {len(batches)} batches, each containing {len(batches[0])} entries???")
    
    return batches

def train_autoencoder(autoencoder, state_list, n_states, batch_size, ae_number):
    ...
    loss_plot = DataPlotter(f"Autoencoder {ae_number} loss", "Mini-batch", "Loss")
