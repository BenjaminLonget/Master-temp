...
class RobotTaskEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = observation=spaces.Box(-10.0, 10.0, shape=observation_shape, dtype=np.float32)#spaces.Dict(
            # dict(
    ...
    def reset(
        ...
        self.ee_position = self.robot.get_ee_position()
        info = {"is_success": self.task.is_success(self.task.get_achieved_goal().astype(np.float32), self.task.get_goal())}
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        observation = self._get_obs()
        new_ee_position = self.robot.get_ee_position()
        # new_ee_position = observation[:3]
        self.sim.create_line(self.ee_position, new_ee_position)
        # self.sim.add_debug_line(self.ee_position, new_ee_position, [1, 0, 0])
        self.ee_position = new_ee_position
        # An episode is terminated if the agent has reached the target
        self.current_step += 1
        done = bool(self.task.is_success(self.task.get_achieved_goal().astype(np.float32), self.task.get_goal()) or collision
                    or self.current_step >= self.max_episode_steps)
        truncated = False
        info = {"is_success": not collision if done else done}
