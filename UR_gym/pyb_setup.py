...
class PyBullet:
    ...
    def __init__(self, render: bool = False, render_mode="rgb_array", n_substeps: int = 20,
                 background_color: Optional[np.ndarray] = None, renderer: str = "Tiny") -> None:
        background_color = background_color if background_color is not None else np.array([96.0, 96.0, 96.0])
        ...
        if self.render_mode == "human":
            ...
            if renderer == "Tiny":
                ...
                raise ValueError("Renderer must be either 'Tiny' or 'OpenGL'")
        else:
        ...
        self.timestep = 1.0 / 500.0 # this controls the speed of the simulation
    ...
    def get_link_distances(self):
        ...
        for link_num in range(5):
            # From link 2-6 represents: upperarm link, forearm link, wrist1, wrist2, wrist3, verified in robot_show.py
            ...
            info = p.getClosestPoints(self._bodies_idx["UR5"], self._bodies_idx["obstacle"], linkIndexA=link_num+2,
                                      distance=5.0)
    ...
    def create_line(self, line_start, line_env):
        """Create a line.

        Args:
            line_start (np.ndarray): The start position, as (x, y, z).
            line_env (np.ndarray): The end position, as (x, y, z).
        """
        self.physics_client.addUserDebugLine(line_start, line_env, lineColorRGB=[1, 0, 0], lineWidth=5)
